...
async def process_scheme(
    ...
    result = {"success": False, "doc_id": doc_id, "status": "processing", "slack_ts": None, "error": None}
    ...
    update_scheme_entry(
        db, doc_id, {"pipeline_status": "processing", "pipeline_started_at": datetime.now(timezone.utc).isoformat()}
    )
    ...
    try:
        ...
        if scraped_text:
            logger.info("Step 3: Extracting contacts with regex")
        ...
        if address:
            logger.info("Step 4: Getting planning area for address")
            try:
