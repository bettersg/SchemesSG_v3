...
from utils.pagination import encode_cursor, decode_cursor, get_paginated_results
...
def test_get_paginated_results_single_page():
    ...
    # Create test data
    results = [
        {"id": f"scheme-{i}", "Similarity": 1.0 - (i * 0.1)} 
        for i in range(5)
    ]
...
def test_get_paginated_results_multiple_pages():
    ...
    # Create test data
    results = [
        {"id": f"scheme-{i}", "scheme_id": f"scheme-{i}", "Similarity": 1.0 - (i * 0.1)} 
        for i in range(25)
    ]
    ...
    for i in range(24):
        assert all_results[i]["Similarity"] >= all_results[i+1]["Similarity"] 