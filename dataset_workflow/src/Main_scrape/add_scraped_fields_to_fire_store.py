...
def add_scraped_fields_to_fire_store(db, doc_ids=None):
    ...
    if doc_ids is None or len(doc_ids)==0:
        ...
        hard_overwrite = True # If specific doc_ids are provided, we do a hard overwrite
    ...
    for doc_id in doc_ids:
        ...
        # NEW: Always check and clean llm_description field if it exists, regardless of refresh status
        existing_llm_description = doc_data.get("llm_description")
        if existing_llm_description and isinstance(existing_llm_description, str):
            # Check if the existing llm_description has forward slash n issues and always clean it
            if '/n' in existing_llm_description or '\\n' in existing_llm_description or '\r' in existing_llm_description:
                cleaned_description = text_extract.clean_llm_description_text(existing_llm_description)
                if cleaned_description != existing_llm_description:
                    update_data = {"llm_description": cleaned_description}
                    doc_ref.update(update_data)
                    logger.info(f"Cleaned existing llm_description for document {doc_id}")

        # Skip if no refresh needed and no cleaning required
        if fields_populated and not require_refresh:
            if not hard_overwrite:
                logger.info(f"Skipping extraction for document {doc_id} as essential fields are already populated and clean.")
