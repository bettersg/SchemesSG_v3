...
export default function ContributePage() {
  ...
  const validateForm = () => {
    if (!formData.Scheme?.trim()) {
      setSubmitStatus({ type: "error", message: "Please enter the scheme name." });
      return false;
    }
    if (!formData.Link?.trim()) {
      setSubmitStatus({ type: "error", message: "Please enter the scheme link." });
      return false;
    }
    try {
      new URL(formData.Link);
    } catch {
      setSubmitStatus({ type: "error", message: "Please enter a valid URL (e.g., https://example.com)." });
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmitStatus({ type: "error", message: data.message || "Failed to submit. Please try again." });
      }
    } catch {
  ...
  return (
    <div className="w-full overflow-y-auto">
      <div className={clsx("max-w-[400px] sm:max-w-[600px]", "mx-auto p-2 sm:p-4")}>
          ...
          <Card.Content>
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              <div className="bg-blue-50 p-4 rounded-lg text-sm">
                ...
                <p className="mt-3 text-gray-500 text-xs">Expected turnaround: ~1 week</p>
              </div>
