function handle__schemespal() {
    ...
    .then(function (resp) {
      ...
      if (resp.mh < 0.55) {
        for (var i = 0; i < resp.data.length; i++) {
          ...
          codeblock =
            '<div class="col-lg-12"><div class="card text-center hover-translate-y-n10 hover-shadow-lg"><div class="px-3 pb-5 pt-5"><div class="py-4"><div class="icon text-warning icon-sm mx-auto"><img height="100" width="100" src="' +
            image +
            '"></div></div><h5 class="">' +
            scheme +
            '</h5><h6 class=" mt-2 mb-0">' +
            agency +
            '</h6><p class=" mt-2 mb-0">Relevance Score: ' +
            relevance +
            '</p><p class=" mt-2 mb-0">' +
            description +
            '</p><div class="mt-4"><div class="mt-4"><a href="' +
            link +
            '" target="_blank" class="link-underline-warning"> Visit Site </a></div></div></div></div>';
      ...
      if (resp.data.length == 0) {
        $("#filler").append(
          '<div class="text-center"><h4>No results matching your search.</h4></div><br><br>'
        );
      }
      $("#filler")
        .append(
          '<div class="text-center"><a class="btn btn-outline-danger text-center" href="schemesbank.html"> Searches not matching your needs? You can <strong>explore our repository manually</strong> </a> <br><br> <a class="btn btn-outline-danger text-center" href="schemescase.html"> Or you can <strong>engage our volunteers</strong>. </a></div>'
        )
        .fadeIn(1000);
    })
    .catch(function (err) {
  ...
  fetch(scriptURLpalquery, {
    method: "POST",
    body: new FormData(schemespalform),
  })
